    ...
    IPublisher publisher) : IRequestHandler<CheckoutCommand, Result<CheckoutResultDto>>
{
    public async Task<Result<CheckoutResultDto>> Handle(CheckoutCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(MapToDto(cart, items));
    }

    private static CheckoutResultDto MapToDto(Cart cart, IReadOnlyList<CartItem> items) => new(
        cart.CartId,
        cart.CustomerId,
        items.Select(i => new CartItemDto(i.CartItemId, i.ProductId, i.ProductName, i.Price, i.Quantity)).ToList());
}
