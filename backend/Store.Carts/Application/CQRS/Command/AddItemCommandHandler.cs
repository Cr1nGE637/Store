    ...
    ICartUnitOfWork unitOfWork) : IRequestHandler<AddItemCommand, Result<GetCartDto>>
{
    ...
    public async Task<Result<GetCartDto>> Handle(AddItemCommand request, CancellationToken cancellationToken)
    {
        ...
        if (saveResult.IsFailure && isNew && saveResult.Error == CartAlreadyExistsForCustomer)
        {
            var existingCartResult = await cartRepository.GetByCustomerIdAsync(request.CustomerId);
            if (existingCartResult.IsFailure)
            ...
            saveResult = await cartRepository.UpdateAsync(cart);
        }
