    ...
    ILogger<OrderStockReservationRequestedEventHandler> logger) : INotificationHandler<OrderStockReservationRequestedEvent>
{
    ...
    public async Task Handle(OrderStockReservationRequestedEvent notification, CancellationToken cancellationToken)
    {
        ...
        var requestedItems = notification.Items
            ...
            .Select(group => new ReservationRequest(
                group.Key,
        ...
        {
            await RejectAsync(notification, "Reservation quantity must be positive", cancellationToken);
            return;
        }

        var stockItems = new List<(ReservationRequest OrderItem, StockItem StockItem)>();
        ...
        {
            var reserveResult = stockItem.Reserve(notification.OrderId, item.Quantity);
        ...
        await outbox.AddAsync([new OrderStockReservedEvent(
            ...
            requestedItems.Select(ToReservedStockItem).ToList())], cancellationToken);
    ...
    private static ReservedStockItem ToReservedStockItem(ReservationRequest item) =>
        new(item.ProductId, item.ProductName, item.Price, item.Quantity);

    private sealed record ReservationRequest(Guid ProductId, string ProductName, decimal Price, int Quantity);
}
