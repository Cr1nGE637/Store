...
public class MarkOrderPaidCommandHandlerTests
{
    ...
    public async Task Handle_WhenCustomerOwnsOrder_MarksOrderPaidAndPublishesEvent()
    {
        ...
        var handler = new MarkOrderPaidCommandHandler(repository, new FakeOrderingUnitOfWork(), outbox, paymentGateway);
        ...
        Assert.Equal("fake-transaction", order.PaymentTransactionId);
        Assert.Equal(order.TotalAmount, paymentGateway.LastAmount);
    ...
    public async Task Handle_WhenCustomerDoesNotOwnOrder_ReturnsAccessDenied()
    {
        ...
        Assert.Equal(0, paymentGateway.CaptureCallCount);
        Assert.Empty(outbox.DomainEvents);
    }

    [Fact]
    public async Task Handle_WhenOrderIsAwaitingStock_ReturnsFailureWithoutCapturingPayment()
    {
        var order = CreateAwaitingStockOrder(Guid.NewGuid());
        var repository = new FakeOrderRepository(order);
        var outbox = new FakeOrderingDomainEventOutbox();
        var paymentGateway = new FakePaymentGateway();
        var handler = new MarkOrderPaidCommandHandler(repository, new FakeOrderingUnitOfWork(), outbox, paymentGateway);
        ...
        Assert.True(result.IsFailure);
        Assert.Equal("Order is awaiting stock reservation", result.Error);
        Assert.Equal(0, paymentGateway.CaptureCallCount);
        Assert.Empty(outbox.DomainEvents);
    }

    [Fact]
    public async Task Handle_WhenOrderIsAlreadyPaid_IsIdempotentWithoutCapturingPaymentAgain()
    {
        var order = CreateUnpaidOrder(Guid.NewGuid());
        order.MarkAsPaid(order.TotalAmount, "existing-transaction");
    ...
    {
        var order = CreateAwaitingStockOrder(customerId);
        order.ConfirmStockReserved();
        order.ClearDomainEvents();
        return order;
    }

    private static Order CreateAwaitingStockOrder(Guid customerId)
    {
    ...
    private sealed class FakePaymentGateway : IPaymentGateway
    {
            ...
            CancellationToken cancellationToken)
        {
            CaptureCallCount++;
            LastAmount = amount;
            return Task.FromResult(Result.Success(new PaymentResult(amount, "fake-transaction")));
        }
    }
}
